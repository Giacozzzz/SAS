...
public class EventManager {
     ...
     * @param chef      Chef assigned to the event
     * @param organizer User organizing the event
     * @return The newly created event
     */
    public Event createEvent(String name, Date dateStart, Date dateEnd, User chef, User organizer) {
        try {
    ...
    public Note addNote(Event ev, String note){
        currentNote = Note.create(ev, ev.getOrg(), note);
        ev.setNote(currentNote);
    ...
    public void removeNote(Note n){
        currentNote = n;
        selectedEvent = n.getEv();
        selectedEvent.setNote(null);

        notifyNoteRemoved();
    }

    public Note modifyNote(Note n, String note){
        n.setNote(note);
        currentNote = n;
    ...
    public ArrayList<Event> getEventBook(){
        ArrayList<Event> eventBook = getEvents();

        eventBook.removeIf(ev -> !ev.hasTerminated());
        return eventBook;
    }
    ...
    private void notifyNoteAdded(){
        for (EventReceiver receiver : eventReceivers){
            receiver.updateNoteAdded(currentNote);
        }
    }

    private void notifyNoteRemoved() {
        for (EventReceiver receiver : eventReceivers) {
            receiver.updateNoteRemoved(currentNote);
        }
    }

    public void notifyNoteModified(){
        for (EventReceiver receiver : eventReceivers) {
            receiver.updateNoteModified(currentNote);
        }
    }
}
